...
**未完成 / 待辦**
- **Jason 待辦**：①打票交所高雄分所 (07)291-5035 問單次查詢費用、買 500 元查詢卡跑通 SOP（文案已對外承諾）；②LINE 官方帳號的**歡迎訊息＋圖文選單**要配合改（站內做到位、加進來卻是制式歡迎詞的話前面全白做）；③GSC API 開通（見下）。
- **觀察節奏**：24h 看 `line_add_click` 有無累積、哪個入口最有效；1 週看「點擊數 vs LINE 實際新增好友數」的落差——落差大＝LINE 那端有問題，而不是站內。
- GSC 程式化讀取：同一個服務帳戶只要 ①啟用 Search Console API ②在 GSC「設定→使用者和權限」加 `ga4-reader@...` 為受限使用者，即可比照 GA4 直接查（本次已測，目前回 403 因 API 未啟用）。
- 內容佇列：**09/02 起斷稿**（`jin-zhi-bei-shu-zhuan-rang` 排在 09/01 是最後一篇），第六批請在 08/28 前備好。
