...
**起點：Jason 說「拉一下 GSC 跟 GA4 的數據」——結果變成整條轉換鏈的重建**

### 一、數據盤查（先看清楚再動手）

**GSC 近 28 天（vs 前期）**：點擊 190 → **772（+306%）**、曝光 9,211 → 33,871、平均排名 8.5 → 7.0、關鍵字 422 → 818。第 1 頁佔比 57%。8/24~25 的 sitemap 解凍與 robots.txt 解封確實生效了。
- ⚠️ **但漲的全是資訊詞，商業詞一個都沒進第一頁**：支票貼現 10.1 名／1 click，支票貸款 15.7、票貼 23.0 皆 0 click，企業融資與企業貸款未進榜。
- 流量高度集中：`zhi-piao-xie-cuo` 一篇吃掉 237 clicks ≈ 全站 31%。

**GA4 近 30 天（台灣）**：1,174 sessions、96% 自然搜尋。

**🔴 發現一：轉換數一直是 0，而且量錯東西**
- 關鍵事件只有 `jf___送出諮詢`（舊 WordPress 表單外掛，現站根本不送）與 `purchase`（電商預設，本站無電商）。**`line_add_click` 與 `generate_lead` 從來沒被標記成關鍵事件。**
- `cta_location` 也沒註冊成自訂維度 → 8/19 埋的點全部查不到。⚠️ **自訂維度與關鍵事件都不回溯，那一個月的分入口資料永久拿不回來。**

**🔴 發現二：這個網站實質上是 68 個各自為政的落地頁，不是一個網站**
- 全站 1,158 pageviews，**非文章頁只佔 19（1.6%）**：三個服務頁合計 5 PV、`/contact` **0**、`/fei-lv-ji-suan` **0**。**頁/次 = 1.05**。
- ⚠️ **這推翻了我自己前一輪給的建議。** 我原本提「把支票寫錯那群人用內鏈導向有錢味的文章」做二段式轉換——數據顯示內鏈點擊率趨近於零，吞吐量約 1%，**那條路是死的**。
- ⚠️ 同時推翻另一個判斷：**表單 0 送出不是表單設計的問題**（我原本懷疑必填欄位太多），是沒有人到得了 `/contact`。
- **結論：文章頁是唯一會被看見的介面，轉換必須在落地頁當場完成。**

**發現三：客群非常精準**——週一～五 194–220 sessions、週六日 55–70，尖峰 9–17 時（最高 15 時）。＝上班時間、在辦公室、手上正拿著一張有問題的支票的人。

**發現四：機器人殘留**——`(not set)` 到達頁 87 sessions／參與率 2.3%／0 PV。**算轉換率要用 ~1,087 當分母，不是 1,174。**

### 二、依 Jason 指示按順序執行

**① CTA 依主題切換 ＋ 文中補位 ＋ 曝光埋點**（`ef1a027`）
- 原本全站只有一個誘因「免費查票信」，但流量第一的 `zhi-piao-xie-cuo`（31%）讀者是**開票的人在修填寫錯誤**，不是收票的人擔心跳票——誘因零關聯。
- 切成 4 種（`src/lib/cta-offers.ts`）：`piao-xin`／`tie-xian`（預設）／`tian-xie`／`zhou-zhuan`。**履行方式完全相同**（LINE 傳一張支票照片、回答一個問題），對 Jason 不是四種新服務。
- `ctaVariantFor()` 依 slug＋關鍵字自動分派，靜態與 DB 排程文共用，**不需新增資料庫欄位、新文章不必手動指定**。129 篇實測 64/36/20/9。
- ⚠️ **比對必須兩段式（slug 全域優先於中文詞）**：單段式時「託收」會把 `zhi-piao-ru-zhang-shi-jian`、`zhi-piao-dui-xian-shou-xu-fei`（流量第 2、3 名）誤判成填寫疑難。改規則後務必重跑全站分派再上線。
- 新增 `cta_view` 曝光事件——只有點擊數無法分辨「沒人看到」（改位置）與「看到不想點」（改文案），**兩者解法相反**。

**② 費率試算搬進文章**（`806c7b9`）
- 抽成 `DiscountCalculator`（頁面版與內嵌版共用同一份公式），28 篇談錢／票期的文章就地嵌入。外幣支票排除（台幣試算套不上）。
- 新增 `calculator_result` 事件（真的算出數字＝意圖最高的訊號）。

**③ 手機修正**（`7febd46`）
- 手機 543 sessions、參與率 41%／33 秒，桌機 61%／71 秒。差距在**每一頁、每個瀏覽器、iOS 與 Android 上都一致** → 不是某頁壞掉。
- **排除效能**：正式站 4x CPU 降速 + Fast 3G 下 TTFB 274ms、FCP 1.36s、185KB。載入沒問題。
- **真因是閱讀路程**：同篇文章手機 15.6 屏、桌機 8.1 屏；hero 佔手機第一屏 63%；行高 2.0 對中文偏鬆。
- 最關鍵：手機平均只待 22 秒＝約看完摘要就走，而文中 CTA 在 50% 處是**手機第 8 屏**，那群人到不了。加了手機專屬早期 CTA，**第一個可見 CTA 由第 6.3 屏提前到第 1 屏**。
- ⚠️ **桌機刻意完全不動**（本來就健康），全部掛 `md:` 斷點，上線後量測確認桌機三項數字一字未改。

### 三、GA4 設定（`26d0491` / `d0a8247`）
`scripts/ga4_setup.py`（預設 dry-run，`--apply` 才寫）建立 `cta_location`／`cta_variant` 自訂維度、把 `line_add_click`／`generate_lead` 標為關鍵事件、`--disable-stale` 取消 `jf___送出諮詢` 的標記（事件與歷史資料保留）。

### 未完成／待驗證
- **這一輪全部是「架好量測、等資料」**，成效一筆都還沒進來（維度不回溯，9/06 之後才開始累積）。
- 📅 約 **2026-10-06** 驗收 `頁/次` 有沒有離開 1.05 —— 決定要不要永久放棄內鏈導流策略。
- 手機仍需捲 15.2 屏，**CSS 治不好**（根因是文章本身 8 分鐘長），要解得從內容結構下手。先看早期 CTA 的數據再決定值不值得。
- 商業大詞（支票貼現／支票貸款／票貼）仍卡在第 1 頁邊緣到第 3 頁，這一輪完全沒碰。
