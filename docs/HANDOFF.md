...
- **GA4 設定補齊（2026-09-06，`scripts/ga4_setup.py`）**：用 Analytics Admin API 建立，非手點。
  ...
  - 🔑 **`ga4-reader@...` 為「編輯者」並維持**（Jason 2026-09-06 決定保留，方便後續直接用 API 改 GA4 設定，不必每次手動升降權）。→ `ga4_setup.py --apply` 可正常寫入。⚠️ 這把金鑰能改也能刪 GA4 設定（維度、關鍵事件、資料串流）；**名稱叫 `reader` 但實為 editor**，看到名字別誤判權限。金鑰在 `~/.config/ga4/`、chmod 600、repo 外。
