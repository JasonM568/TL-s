...
⚠️ 權限：讀取（不加 --apply）只要「檢視者」；**寫入需要「編輯者」**。
   ga4-reader@huangxi-analytics.iam.gserviceaccount.com 於 2026-09-06 升為
   「編輯者」並**維持在此**（Jason 2026-09-06 決定保留，方便後續直接改設定）。
   → --apply 可正常寫入。
   ⚠️ 這把金鑰能改也能刪 GA4 設定（維度、關鍵事件、資料串流）。
      名稱叫 reader 但實為 editor，看到名字別誤判權限。
   ⚠️ 升降權本腳本做不到：GA4 使用者管理需「管理員」角色，編輯者不含此權限
      （accessBindings 一律 403），只能在後台手動點。
